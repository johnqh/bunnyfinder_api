 ...
 *
 * Creates the following in the `bunnyfinder` schema (all operations are idempotent):
 * 1. The `bunnyfinder` schema itself (`CREATE SCHEMA IF NOT EXISTS`)
 * 2. The `bunnyfinder.users` table with columns: `firebase_uid` (PK), `email`,
 *    `display_name`, `created_at`, `updated_at`
 * 3. The `bunnyfinder.histories` table with columns: `id` (UUID PK), `user_id` (FK to users),
 *    `datetime`, `value` (numeric 12,2), `created_at`, `updated_at`
 * 4. An index on `bunnyfinder.histories(user_id)` for efficient user-scoped queries
 *
...
export async function initDatabase() {
  ...
  await client`CREATE SCHEMA IF NOT EXISTS bunnyfinder`;

  await client`
    CREATE TABLE IF NOT EXISTS bunnyfinder.users (
  ...
  await client`
    CREATE TABLE IF NOT EXISTS bunnyfinder.entities (
  ...
  await client`
    CREATE TABLE IF NOT EXISTS bunnyfinder.entity_members (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      entity_id UUID NOT NULL REFERENCES bunnyfinder.entities(id) ON DELETE CASCADE,
  ...
  await client`
    CREATE UNIQUE INDEX IF NOT EXISTS bunnyfinder_entity_members_entity_user_idx
    ON bunnyfinder.entity_members(entity_id, user_id)
  `;

  await client`
    CREATE TABLE IF NOT EXISTS bunnyfinder.entity_invitations (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      entity_id UUID NOT NULL REFERENCES bunnyfinder.entities(id) ON DELETE CASCADE,
  ...
  await client`
    CREATE TABLE IF NOT EXISTS bunnyfinder.histories (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      user_id VARCHAR(128) NOT NULL REFERENCES bunnyfinder.users(firebase_uid) ON DELETE CASCADE,
      entity_id UUID NOT NULL REFERENCES bunnyfinder.entities(id) ON DELETE CASCADE,
  ...
  await client`
    ALTER TABLE bunnyfinder.histories
    ADD COLUMN IF NOT EXISTS entity_id UUID REFERENCES bunnyfinder.entities(id) ON DELETE CASCADE
  `;

  await client`
    CREATE INDEX IF NOT EXISTS bunnyfinder_histories_user_idx
    ON bunnyfinder.histories(user_id)
  `;

  await client`
    CREATE INDEX IF NOT EXISTS bunnyfinder_histories_entity_idx
    ON bunnyfinder.histories(entity_id)
  `;
