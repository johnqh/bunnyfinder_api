...
Backend API server for the BunnyFinder application.

**Package**: `entitybunnyfinder_api` (private, not published)
...
- **entitybunnyfinder_types** — Shared TypeScript type definitions; this project imports request/response types and helpers from it
- **entitybunnyfinder_client** — API client SDK that consumes this server's HTTP endpoints
- **entitybunnyfinder_lib** — Business logic library that uses entitybunnyfinder_client to talk to this API
- **entitybunnyfinder_app** — Web frontend that calls this API (both default to port 8022)
- **entityentitybunnyfinder_app_rn** — React Native app that calls this API
...
- All API responses must be wrapped in `BaseResponse<T>` using `successResponse()` or `errorResponse()` from `@sudobility/entitybunnyfinder_types`
...
- The default port is `8022`, matching `entitybunnyfinder_app`'s default `VITE_API_URL` of `localhost:8022`
